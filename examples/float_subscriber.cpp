...
int main(int argc, char * argv[])
{
  ...
  const auto node = std::make_shared<rclcpp::Node>("float_subscriber");
  ...
  try {
    subscriber = std::make_shared<dynamic_interface::Subscriber>(
      ...
      [](const boost::json::value & data) {
        const double value = data.at("data").as_double();
        RCLCPP_INFO(
          rclcpp::get_logger("FloatSubscriber"),
          "Message received: {\"data\": %.2f}", value);});
